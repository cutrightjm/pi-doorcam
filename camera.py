...
def upload_old():
    ...
    for filename in os.listdir(temp_dir):
        if filename.endswith(".h264"):
            old_file = (os.path.join(temp_dir, filename))
            try:
                final_destination = upload_destination + filename
                scp_command = ['scp', '-i', privkey_location, old_file, upload_path]
                print("... Uploading old file " + old_file)
                subprocess.call(scp_command)
                print("... Deleting old file...")
                delete_command = ['rm', old_file]
...
while True:
    ...
    if button.is_pressed:
        if isRecording == 0:
            ...
            time.sleep(0.6)
            stdin, stdout, stderr=ssh_beep.exec_command(open_beep)
        ...
        kill_beep = 'pkill -f play'
        stdin,stdout,stderr=ssh_beep.exec_command(kill_beep)
        if isRecording == 1:
            isRecording = 0
            ...
            try:
                ...
                print("... The file name was " + file_name)
